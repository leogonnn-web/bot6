...
**Срез на 2026-06-11 13:35 UTC (≈69.5 ч / 2.9 дня сбора):**
| Метрика | Значение |
|---------|----------|
| Размечено сэмплов (profit ≠ null) | **486** (из 1597 строк; 486 исполненных позиций) |
| conservative labeled | **398 / 500** (~80%, до цели <суток) |
| По режимам | conservative 82.7% · normal 16.7% · red_light **0.6% (10)** |
| Win-rate | **90.8%** (по закрытиям) / 89.3% (labeled) |
| Realized PnL (dry-run) | **+$49.43** (~+1.7%/день, верхняя граница) |
| Частота | **~7.0 позиций/час ≈ 168/день** |
| Avg score | 1.68 (cons 1.60 / normal 2.09 / red_light 0.90) |
| Паник-выходы | 41 / 565 (7.3%), −$9.83 |
| `rvol_spike` avg | 10.84 (рос с 5.96 — рынок волатильнее) |

**Выводы:** порог калибровки (≥50) пройден; до 500 conservative <суток → скоро первый
прогон `tools/calibrate_dispatcher.py` (П.4). `red_light` появился (10), но для его
режимных весов (П.1) пока мало. Win-rate держится ~90% даже при росте волатильности.
Dry-run завышает PnL (нет слиппеджа) — нужен месяц разных рыночных условий.

**Предыдущий срез (2026-06-09, 18.5ч):** 143 labeled, WR 91.9%, +$15.80, ~187/день, red_light=0.
